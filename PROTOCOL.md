...
## ROUTE — загрузка маршрута для автономного режима

Характеристика `6e6f7604-…`, Write. Телефон загружает весь маршрут в память
устройства; дальше устройство ведёт по своему GPS-модулю без телефона.

Сообщения (первый байт — тип):

| Тип | Формат | Назначение |
|---|---|---|
| 1 | `[1, nLo, nHi]` | начало: сколько всего точек (сброс буфера) |
| 2 | `[2, idxLo, idxHi, cnt, cnt×(int32 lat·1e6 LE, int32 lon·1e6 LE)]` | блок точек маршрута с индекса idx |
| 3 | `[3, cnt, cnt×(idxLo, idxHi, kind)]` | манёвры: привязка к индексу точки + тип |
| 4 | `[4]` | завершить: посчитать дистанции, включить автономный режим |

Точки шлются по 2 за запись (одна запись ≤ 20 байт — влезает в дефолтный
BLE MTU). Для ~330 точек это ~170 записей, один раз перед поездкой.

### Автономный режим (устройство)

- Нет телефона + маршрут загружен + есть GPS-фикс → устройство само считает
  позицию на маршруте, форму дороги впереди, дистанцию до манёвра и прогресс
  (та же отрисовка, что для NAV) и рисует зелёную точку-индикатор слева.
- Нет фикса → заставка «searching GPS…».
- Дальше 50 м от линии → «OFF ROUTE» (офлайн-перестроения нет — нужна карта).
- GPS-модуль: u-blox NEO-6M/7M/8M по UART, NMEA, 9600 бод (TinyGPSPlus).
