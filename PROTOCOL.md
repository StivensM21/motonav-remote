...
## NAV v3 — навигация с формой дороги и прогрессом (7 + 2·N байт)

Страница шлёт раз в секунду. Байт 0 = 3 отличает формат; прошивка понимает
также v2 (без прогресса) и старый 10-байтовый v1 (только стрелка).

| Байт | Значение |
|---|---|
| 0 | версия протокола = 3 |
| 1 | манёвр: 0 прямо, 1 направо, 2 налево, 3 круг, 4 разворот, 5 финиш |
| 2–3 | метров до манёвра, uint16 LE |
| 4 | скорость, км/ч |
| 5 | прогресс маршрута, % (0–100) — для дуги по нижнему краю экрана |
| 6 | N — число точек дороги (0–16) |
| 7… | N пар int8: (x, y) — форма маршрута впереди в пикселях экрана, относительно якоря-шеврона; x вправо, y вперёд по курсу; масштаб 0.45 px/м (~450 м обзора) |
