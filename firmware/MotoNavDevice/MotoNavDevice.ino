...
#include <esp_sleep.h>
#include <TinyGPSPlus.h>   // Менеджер библиотек: "TinyGPSPlus" (Mikal Hart)
...
// ---- GPS-модуль (u-blox NEO-6M/7M/8M, UART/NMEA) ----
// ПРОВЕРЬ распиновку под свою плату! По умолчанию — свободные пады TX/RX.
// GPS TX → сюда (RX платы); GPS RX → сюда (TX платы); GPS VCC → 3V3/5V; GND → GND.
#define GPS_RX_PIN   44   // RX платы  ← TX модуля GPS
#define GPS_TX_PIN   43   // TX платы  → RX модуля GPS
...
#define ROUTE_UUID "6e6f7604-b5a3-f393-e0a9-e50e24dcca9e"  // загрузка маршрута (write)
...
// ---- автономный режим: маршрут загружен в устройство, ведём по своему GPS ----
#define MAXRP 350                // точек маршрута (телефон прореживает до этого)
#define MAXRM 48                 // манёвров
float   rpLat[MAXRP], rpLon[MAXRP], rpCum[MAXRP];
int     routeN = 0;
uint16_t manAtIdx[MAXRM];        // манёвр привязан к индексу точки маршрута
uint8_t  manKind[MAXRM];
int     manCount = 0;
bool    haveRoute = false;       // маршрут загружен и готов
int     rpUploadExpect = 0;      // сколько точек ждём при загрузке
int     nearIdx = 0;             // ближайшая точка маршрута под нами
bool    gpsFix = false;
double  gpsLat = 0, gpsLon = 0, gpsCourse = 0, gpsKmh = 0;
bool    offRoute = false;
...
// ---- гео-функции для автономного режима ----
double geoHaversine(double a, double b, double c, double d) {
  const double R = 6371000, r = M_PI / 180;
  double dLat = (c - a) * r, dLon = (d - b) * r;
  double s = sin(dLat/2)*sin(dLat/2) + cos(a*r)*cos(c*r)*sin(dLon/2)*sin(dLon/2);
  return 2 * R * asin(sqrt(s));
}
double geoBearing(double la1, double lo1, double la2, double lo2) {
  const double r = M_PI / 180;
  double y = sin((lo2-lo1)*r) * cos(la2*r);
  double x = cos(la1*r)*sin(la2*r) - sin(la1*r)*cos(la2*r)*cos((lo2-lo1)*r);
  double b = atan2(y, x) / r;
  return b < 0 ? b + 360 : b;
}

// ---- загрузка маршрута с телефона (характеристика ROUTE) ----
// формат: [1,nLo,nHi] начало · [2,idxLo,idxHi,cnt, cnt×(int32 lat_e6, int32 lon_e6)] точки
//         [3,cnt, cnt×(idxLo,idxHi,kind)] манёвры · [4] завершить
class RouteCB : public BLECharacteristicCallbacks {
  void onWrite(BLECharacteristic *c) override {
    uint8_t *d = c->getData();
    size_t n = c->getLength();
    if (n < 1) return;
    if (d[0] == 1 && n >= 3) {                       // начало
      routeN = 0; manCount = 0; haveRoute = false; offRoute = false;
      rpUploadExpect = d[1] | (d[2] << 8);
    } else if (d[0] == 2 && n >= 4) {                // блок точек
      int idx = d[1] | (d[2] << 8), cnt = d[3];
      for (int i = 0; i < cnt; i++) {
        int o = 4 + i * 8;
        if (o + 8 > (int)n) break;
        int32_t la = (int32_t)(d[o] | (d[o+1]<<8) | (d[o+2]<<16) | (d[o+3]<<24));
        int32_t lo = (int32_t)(d[o+4] | (d[o+5]<<8) | (d[o+6]<<16) | (d[o+7]<<24));
        int j = idx + i;
        if (j >= 0 && j < MAXRP) { rpLat[j] = la / 1e6; rpLon[j] = lo / 1e6; if (j + 1 > routeN) routeN = j + 1; }
      }
    } else if (d[0] == 3 && n >= 2) {                // манёвры
      int cnt = d[1];
      for (int i = 0; i < cnt && manCount < MAXRM; i++) {
        int o = 2 + i * 3;
        if (o + 3 > (int)n) break;
        manAtIdx[manCount] = d[o] | (d[o+1] << 8);
        manKind[manCount]  = d[o+2];
        manCount++;
      }
    } else if (d[0] == 4) {                          // завершить: посчитать дистанции
      rpCum[0] = 0;
      for (int i = 1; i < routeN; i++)
        rpCum[i] = rpCum[i-1] + geoHaversine(rpLat[i-1], rpLon[i-1], rpLat[i], rpLon[i]);
      nearIdx = 0;
      haveRoute = (routeN >= 2);
      lastActivity = millis();
    }
...
void drawScreen() {
  ...
  bool auton = (!connected && haveRoute);          // автономный режим: маршрут без телефона

  // заставки (вариант 2): локатор с радар-волнами + GPS
  if (!connected && !haveRoute) { drawSplash("Ready for the trip", C_DIM); return; }
  if (auton && !gpsFix)         { drawSplash("searching GPS...", C_DIM);    return; }
  if (connected && navAt == 0)  { drawSplash("connected", C_MINT);          return; }
  ...
  // автономно и сошли с маршрута — предупреждение
  if (auton && offRoute) {
    gfx->fillRect(28, 90, 184, 26, C_BG);
    textAt(120, 98, "OFF ROUTE", 2, C_RED);
  }
  // маленький значок автономного режима (без телефона)
  if (auton) { gfx->fillCircle(26, 120, 4, C_MINT); }
}
...
void setup() {
  ...
  BLECharacteristic *routeChr = svc->createCharacteristic(
      ROUTE_UUID, BLECharacteristic::PROPERTY_WRITE_NR | BLECharacteristic::PROPERTY_WRITE);
...
// автономная навигация по загруженному маршруту и своему GPS
void computeAutonomous() {
  if (!haveRoute || !gpsFix || routeN < 2) return;
  double la = gpsLat, lo = gpsLon;

  // ближайшая точка маршрута (окно вокруг предыдущей)
  int best = nearIdx; double bestD = 1e12;
  int i0 = max(0, nearIdx - 5), i1 = min(routeN - 1, nearIdx + 80);
  for (int i = i0; i <= i1; i++) {
    double dd = geoHaversine(la, lo, rpLat[i], rpLon[i]);
    if (dd < bestD) { bestD = dd; best = i; }
  }
  nearIdx = best;
  offRoute = bestD > 50;

  // курс: GPS при движении, иначе на следующую точку маршрута
  double hdg;
  if (gpsKmh > 3 && gps.course.isValid()) hdg = gpsCourse;
  else if (best + 1 < routeN)             hdg = geoBearing(la, lo, rpLat[best+1], rpLon[best+1]);
  else                                    hdg = gpsCourse;
  double hr = hdg * M_PI / 180, S = 0.45;

  // ближайший манёвр впереди
  int mi = -1;
  for (int i = 0; i < manCount; i++) if (manAtIdx[i] >= best) { mi = i; break; }
  uint8_t mtype; double mdist;
  double alongToNear = geoHaversine(la, lo, rpLat[best], rpLon[best]);
  if (mi >= 0) {
    int midx = manAtIdx[mi];
    mtype = manKind[mi];
    mdist = alongToNear + (rpCum[midx] - rpCum[best]);
  } else {
    mtype = 5;                                   // финиш
    mdist = alongToNear + (rpCum[routeN-1] - rpCum[best]);
  }
  if (mdist < 0) mdist = 0;

  // форма дороги впереди ~450 м → пиксели, курс вверх
  int cnt = 0; double acc = 0, plat = la, plon = lo;
  for (int i = best + 1; i < routeN && cnt < 16 && acc < 450; i++) {
    double seg = geoHaversine(plat, plon, rpLat[i], rpLon[i]);
    if (seg < 8 && i < routeN - 1) continue;
    acc += seg; plat = rpLat[i]; plon = rpLon[i];
    double e = (rpLon[i] - lo) * cos(la * M_PI / 180) * 111320;
    double nn = (rpLat[i] - la) * 110540;
    double x = (e * cos(hr) - nn * sin(hr)) * S;
    double y = (e * sin(hr) + nn * cos(hr)) * S;
    navPX[cnt] = (int8_t)constrain((int)lround(x), -127, 127);
    navPY[cnt] = (int8_t)constrain((int)lround(y), -127, 127);
    cnt++;
  }
  navN = cnt;

  navType = mtype;
  navDist = (uint16_t)min(65535.0, mdist);
  navSpd  = (uint8_t)min(255.0, gpsKmh);
  double total = rpCum[routeN-1] > 0 ? rpCum[routeN-1] : 1;
  navProg = (uint8_t)constrain((int)lround(rpCum[best] / total * 100), 0, 100);
  navStubN = 0; navMX = -128;
  navAt = millis(); navFresh = true;
}

void loop() {
  static uint32_t lastDraw = 0, lastBatt = 0, lastAuton = 0;
  uint32_t now = millis();
  ...
  if (gps.location.isValid() && gps.location.isUpdated()) {
    ...
    gpsFix = true; lastGpsFixMs = now;
  }
  if (gps.speed.isValid())  gpsKmh    = gps.speed.kmph();
  if (gps.course.isValid()) gpsCourse = gps.course.deg();
  if (gpsFix && now - lastGpsFixMs > 5000) gpsFix = false;   // потеря сигнала

  // автономный режим: без телефона, но маршрут загружен и есть фикс
  if (!connected && haveRoute && gpsFix && now - lastAuton > 300) {
    lastAuton = now;
    computeAutonomous();
    lastActivity = now;                          // не спать во время автономной езды
  }
